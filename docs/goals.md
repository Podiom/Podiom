...
| `cli_tool` | A tool **you** must install host-wide (a system package, brew, apt): approval acknowledges the request and shows you the suggested command, and the agent re-detects the tool at its next review. Podiom installs nothing here — anything an agent can install itself goes into the shared [toolset](toolset.md) with no approval at all, so a request reaching you means it genuinely could not. |
| `env_var` | Requests name the variable and purpose only — **the request never carries the value**. In the approval dialog you may enter the value once: Podiom stores it in `credentials.yaml` (readable only by your OS user) and injects it into agent environments on later runs; it is never shown again and never returned by the API. Leave the field empty to just acknowledge and set the variable yourself where `podiomd` runs. Stored credentials are managed under Settings → Credentials, where you can also add one directly without waiting to be asked. An agent that already *has* a secret does not file a request at all — it stores it itself with `podiom_store_credential`, and the credential shows up attributed to it. |
