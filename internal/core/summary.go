...
func replayHistory(sess store.Session, history []store.Message) []store.Message {
	...
	if sess.RollingSummary == "" || len(history) <= recentReplayMessages {
		return withApprovedPlan(sess, history)
	}
	...
	return withApprovedPlan(sess, compacted)
}

// withApprovedPlan pins an approved plan into every replay.
//
// Without this the plan the user approved can be silently lost in exactly the
// cases where it matters most: a rate-limit fallback starts a fresh backing
// session, and a long implementation pushes the plan out of the recent-replay
// window. Reading it from the session row rather than hunting for the original
// message means it also survives a provider or profile switch for free.
//
// A retained PlanInfo with the gate down is what "approved" looks like: while a
// plan is awaiting approval the state is still PlanAwaitingApproval, and a
// rejected plan clears PlanInfo.
func withApprovedPlan(sess store.Session, history []store.Message) []store.Message {
	if sess.PlanState != store.PlanNone {
		return history
	}
	markdown := strings.TrimSpace(sess.PlanInfo.Markdown)
	if markdown == "" {
		return history
	}
	pinned := "The user approved this implementation plan. Implement it. If you need to deviate, say so and why before doing it."
	if path := strings.TrimSpace(sess.PlanInfo.FilePath); path != "" {
		pinned += "\nThe plan is also saved at: " + path
	}
	out := make([]store.Message, 0, len(history)+1)
	out = append(out, store.Message{Role: store.RoleUser, Content: pinned + "\n\n" + markdown})
...
func deterministicSummary(history []store.Message) string {
	...
	for _, msg := range history {
		...
		line := oneLine(messageTranscriptContent(msg))
		if line == "" {
			continue
		}
